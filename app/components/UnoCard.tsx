...
// Kept in sync with Lib/Properties.ts (label + price only — this component
// doesn't need `value`).
const PROPERTY_META: Record<string, { label: string; price: number }> = {
  property_wendyhouse: { label: "Wendy House", price: 150 },
  property_studio: { label: "Studio Flat", price: 350 },
  property_apartment: { label: "Apartment", price: 600 },
  property_cottage: { label: "Cottage", price: 900 },
  property_house: { label: "Suburban House", price: 1200 },
  property_townhouse: { label: "Townhouse", price: 1600 },
  property_cabin: { label: "Log Cabin", price: 1900 },
  property_condo: { label: "Beach Condo", price: 2000 },
  property_villa: { label: "Villa", price: 2600 },
  property_hotel: { label: "Boutique Hotel", price: 3200 },
  property_penthouse: { label: "Penthouse", price: 3800 },
  property_mansion: { label: "Mansion", price: 5000 },
  property_castle: { label: "Castle", price: 6500 },
  property_island: { label: "Private Island", price: 8500 },
};
...
// Same short-form rule as formatMoney (no sign): 150 -> "$150", 1200 -> "$1.2k"
function formatPrice(n: number) {
  const short = n >= 1000 ? `${Math.round(n / 100) / 10}k` : `${n}`;
  return `$${short}`;
}
...
export function UnoCard({
  ...
  const displayValue = lifeMeta
    ? formatMoney(lifeMeta.amount)
    : propertyMeta
      ? formatPrice(propertyMeta.price)
      : (VALUE_DISPLAY[value] ?? value.toUpperCase());
  const subLabel = lifeMeta?.label ?? propertyMeta?.label;
  const s = SIZE_CLASSES[size];
  const cs = COLOR_STYLES[color] ?? COLOR_STYLES.wild;
  const isWild = color === "wild";
  const isSpecial = color === "life" || color === "property";
  const isProperty = color === "property";

  // ─── Property video header ────────────────────────────────────────────────
  // Mirrors PropertyMediaHeader (used in the accept/decline offer modal) so
  // the same looping clip shows up on the card face itself. Falls back to
  // the plain teal gradient + 🏠 emoji if no video asset exists yet for this
  // property id — never shows a broken video element.
  const propertyMedia = isProperty ? getPropertyMedia(cardId) : null;
  const [videoFailed, setVideoFailed] = useState(false);
  const showPropertyVideo =
    isProperty && !isFaceDown && propertyMedia && !videoFailed;
  ...
  return (
    ...
    >
      ...
      >
        {/* Property cards: looping video header behind everything else,
            same asset PropertyMediaHeader uses in the offer modal. */}
        {showPropertyVideo && (
          <>
            <video
              key={propertyMedia.videoSrc}
              src={propertyMedia.videoSrc}
              autoPlay
              muted
              loop
              playsInline
            ...
            />
            {/* Dark tint so corner labels / oval stay readable over the clip */}
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background:
                  "linear-gradient(180deg, rgba(4,35,32,0.35) 0%, rgba(4,35,32,0.15) 45%, rgba(4,35,32,0.75) 100%)",
              }}
            />
          </>
        )}
        ...
        {!isFaceDown ? (
          <>
            ...
            {/* Center oval — tilted like real UNO cards. Skipped for property
                cards with a live video header so the clip stays unobstructed;
                the price/label already sit in the corners and bottom tint. */}
            {!showPropertyVideo && (
              <div
                className="absolute top-1/2 left-1/2 flex flex-col items-center justify-center"
                style={{
            ...
            {/* Property cards with a video header still show the label near
                the bottom of the card, over the dark tint. */}
            {showPropertyVideo && subLabel && (
              <div
                className="absolute left-0 right-0 flex flex-col items-center justify-end px-1 z-10"
                style={{ bottom: "0.35rem" }}
              >
                <span
                  className="text-white font-black leading-none text-center px-1"
                  style={{
                    fontSize: `calc(${s.center} * 0.7)`,
