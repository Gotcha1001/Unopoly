...
// 1. NEW stage: "choosing" — shown first (once Clerk has loaded the user),
//    before any room/bot/game exists. Replaces the old auto-run setup effect
//    that fired on mount with no way to configure the bot first.
// 2. Room/bot/game creation now happens inside `handleChoose(difficulty)`,
//    triggered by the player picking Easy or Hard — not automatically. The
//    chosen difficulty is passed straight into addBot's new `difficulty` arg
//    ("conservative" for Easy, "aggressive" for Hard).
// 3. Rematch now sends the player back to "choosing" instead of immediately
//    rebuilding with whatever was picked last time, so they can pick again
//    (or the same one) before every game, not just the first.
// 4. `didRun` ref is gone — it existed only to guard the old auto-run
//    useEffect against React Strict Mode's double-invoke. Setup is now
//    click-triggered, so a `settingUp` ref guards against double-clicks
//    instead.
// ─────────────────────────────────────────────────────────────────────────────
